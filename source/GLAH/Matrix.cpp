...
Matrix4x4::GetOrtho(float left, float right, float bottom, float top, float a_fNear, float a_fFar)
{
	...
	toReturn[0] = 2.0f/(right - left);
	toReturn[1] = toReturn[2] = toReturn[3] = toReturn[4] = 0.f;
	toReturn[5] = 2.0f/(top - bottom);
	toReturn[6] = toReturn[7] = toReturn[8] = toReturn[9] = 0.f;
	toReturn[10] = 2.0f/(a_fFar - a_fNear);
	toReturn[11] = 0.f;
